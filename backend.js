(() => {
  ...
  async function saveSettings(settings) { const { appearance, posLayout, language, accent } = settings, appConfig = { options: settings.options || {}, wholesale: Boolean(settings.wholesale), cashClosing: settings.cashClosing || {}, documentTemplates: settings.documentTemplates || {}, watermarkOpacity: settings.watermarkOpacity, email: settings.email || '', address: settings.address || '', postalCode: settings.postalCode || '', website: settings.website || '' }; return upsert('business_settings', { theme: { ...(appearance || { accent }), language: language || 'es' }, pos_layout: posLayout || {}, app_config: appConfig }, 'business_id'); }
