(() => {
  ...
  async function login(email, password) {
    ...
    const memberships = await request(`/rest/v1/memberships?select=business_id,role,display_name,active&user_id=eq.${auth.user.id}&active=is.true&limit=1`, { headers: { Authorization: `Bearer ${auth.access_token}` } });
    if (!memberships?.length) throw new Error('El usuario no está vinculado a una empresa activa');
    const member = memberships[0];
    session = { accessToken: auth.access_token, refreshToken: auth.refresh_token, expiresAt: Date.now() + auth.expires_in * 1000, userId: auth.user.id, email: auth.user.email, name: member.display_name || auth.user.email, role: member.role, businessId: member.business_id };
  ...
  async function loadWorkspace() {
    ...
    return {
      settings: { businessName: business.name || 'GRASSI', legalName: business.legal_name || '', nit: business.tax_id || '', phone: business.phone || '', city: business.city || '', currency: business.currency || 'Bs', accent: settings?.[0]?.theme?.accent || '#0098f9', appearance: settings?.[0]?.theme || {}, posLayout: settings?.[0]?.pos_layout || {} },
      products: products.map(row => ({ ...fromSnake(row), id: row.id, minStock: Number(row.min_stock), wholesale: Number(row.wholesale_price), price: Number(row.price), cost: Number(row.cost), stock: Number(row.stock) })),
      clients: clients.map(row => ({ ...fromSnake(row), id: row.id, balance: Number(row.balance), purchases: Number(row.purchases), total: Number(row.total_purchased), ledger: (ledgerByClient[row.id] || []).map(entry => ({ date: entry.created_at, type: entry.kind, description: entry.description, amount: Number(entry.amount) })) })),
      ...
      sales: sales.map(row => ({ id: row.sale_number, uuid: row.id, date: row.created_at, client: row.client_name || 'Consumidor final', employee: row.seller_name || '', employeeId: row.seller_id, items: (itemsBySale[row.id] || []).reduce((n, x) => n + Number(x.quantity), 0), total: Number(row.total), payment: row.payment_method, type: row.kind || 'Venta' })),
  ...
  async function saveClient(client) { return upsert('clients', { id: client.id.length === 36 ? client.id : undefined, code: client.code, name: client.name, customer_type: client.type, phone: client.phone, document: client.document, city: client.city, balance: client.balance, notes: client.notes }); }
  ...
  async function saveSettings(settings) { return upsert('business_settings', { theme: settings.appearance || { accent: settings.accent }, pos_layout: settings.posLayout || {} }, 'business_id'); }
  ...
  async function registerSale(payload) { if (!isRemote()) return null; return request('/rest/v1/rpc/register_sale', { method: 'POST', body: { p_business_id: businessId, p_client_id: payload.clientId || null, p_payment_method: payload.payment, p_items: payload.items, p_notes: payload.notes || null } }); }
